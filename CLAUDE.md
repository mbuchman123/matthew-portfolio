...
- **Content**: tabbed video grid (`switchTab(id, btn)` swaps `.tab-panel.active`). Cards are **not** hardcoded in `index.html` — they're generated by `renderVideoTabs()` from the `VIDEOS` array in `videos.js` (loaded via `<script src="videos.js">` before the main inline `<script>`). To add/reorder/remove a post, edit `videos.js` only — see the comment at the top of that file for the exact object shape and the three card statuses (`live`, `placeholder`, `soon`). Clicking a `live` card opens the `#video-modal` lightbox (`openVideoModal`/`closeVideoModal`), which embeds via a direct iframe (`tiktok.com/embed/v2/{id}` or `{instagram-url}/embed`) rather than the platforms' oEmbed `<script>` widgets, since those don't re-process cleanly when the modal's content changes dynamically. `placeholder`/`soon` cards have no `id` to embed, so they fall through to a plain link (or no link at all for `soon`).
